//! Windows system speech through WinRT speech synthesis.
...
fn speak_next(
    ...
    let base = active.bases.pop_front().unwrap_or(0);

    match engine.speak(&sentence) {
        Ok(()) => {
            let generation = active.generation;
            let end = base + sentence.text.encode_utf16().count() as u32;
            (callbacks.word_range)((generation, base, end));
            Some(generation)
        }
        Err(error) => {
            let generation = active.generation;
