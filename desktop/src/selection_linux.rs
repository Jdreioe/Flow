...
/// Synchronous wrapper: builds a short-lived runtime so the controller and
/// the iced UI stay free of async plumbing.
pub fn read_focused_selection() -> Result<String, SelectionError> {
    let runtime =
        tokio::runtime::Runtime::new().map_err(|_| SelectionError::Unavailable)?;
    runtime.block_on(read_focused_selection_async())
}

async fn read_focused_selection_async() -> Result<String, SelectionError> {
    ...
    for application in applications {
        ...
        if application_name == "flow-desktop" {
            continue;
        }
...
pub fn enable_accessibility() {
    ...
    runtime.block_on(enable_accessibility_async());
}

async fn enable_accessibility_async() {
